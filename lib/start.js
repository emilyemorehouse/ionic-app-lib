...
Start.addDefaultPlatforms = function addDefaultPlatforms(options) {
  ...
  if (currOs == 'darwin') {
    ...
    .then(function(v) {
      return generate.copyIconFilesIntoResources(false, options.targetPath)
    })
    .then(function() {
      return generate.addIonicIcons('ios');
    })
    .catch(function(ex) {
      logging.logger.debug('ex happened', ex);
      logging.logger.debug(ex.stack);
      throw ex;
    })
  } else {
    return Q();
...
Start.fetchWrapper = function fetchWrapper(options) {
  var q = Q.defer();
  // var self = this;
  var WRAPPER_REPO_NAME = options.v2 ? 'ionic2-app-base' : 'ionic-app-base';
  var repoUrl = 'https://github.com/driftyco/' + WRAPPER_REPO_NAME + '/archive/master.zip';

  Utils.fetchArchive(options.targetPath, repoUrl, options.isGui)
  .then(function() {
...
Start.fetchCreatorApp = function(options) {
  ...
  request({ url: downloadUrl, proxy: proxy }, function(err, res, html) {
    if(!err && res && res.statusCode === 200) {
      ...
      // logging.logger.info(['Unable to fetch', err, res.statusCode].join(' '));
      q.reject(res);
    }
    q.resolve();
  });
...
Start.fetchGithubStarter = function(options, repoUrl) {
  ...
  Utils.fetchArchive(options.targetPath, repoUrl, options.isGui).then(function() {
...
Start.fetchZipStarter = function fetchZipStarter(options) {
  ...
  Utils.fetchArchive(options.targetPath, options.zipFileDownload, options.isGui)
  .then(function() {
...
Start.fetchPlnkr = function fetchPlnkr(options) {
  ...
  Utils.fetchArchive(extractPath, plnkrUrl, options.isGui)
  .then(function() {
    try {
      // Move the content of this repo into the www folder
...
Start.initCordova = function(options, appSetup) {
  ...
  try {
    ...
    logging.logger.info('Adding native plugins'.green.bold);
